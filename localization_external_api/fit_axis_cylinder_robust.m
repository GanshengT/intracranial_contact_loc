...
ip.addParameter('CoherencePercent', 30); % How much % of farthest neighbors to use
ip.addParameter('CosineThresh', 0.97);     % Percentile threshold for voxel inclusion
ip.addParameter('subj_id', []);
ip.addParameter('report_dir', []);
ip.addParameter('traj_id', []);

ip.parse(varargin{:});
opt = ip.Results;

% in case P contains voxels from multiple leads, we will do local
% orientation coherence analysis to decide what to exclude
% 	For each voxel:
% Randomly sample K other voxels.
% Compute the vector directions from the voxel to those K.
% Calculate the pairwise cosine similarity between these vectors.
% Average them → gives a “local directional consistency” score per voxel.
N = size(P, 1);

% 
% cosine_stats = voxel_cosine_consistency(P, opt.CoherencePercent);
% cos_vals = cosine_stats.avg_cosine_similarity;
% keep_mask = cos_vals >= opt.CosineThresh;
% P_all = P;
% P = P_all(keep_mask, :);  % Keep only high-coherence voxels
% 
% cmap = turbo;
% cvals = cos_vals;
% 
% figure;
% 
% % Plot excluded voxels first (with black edge for contrast)
% scatter3(P_all(~keep_mask,1), P_all(~keep_mask,2), P_all(~keep_mask,3), ...
%     30, cos_vals(~keep_mask), ...
%     'filled', ...
%     'MarkerEdgeColor', 'k', 'LineWidth', 0.7);
% hold on;
% 
% % Plot kept voxels on top
% scatter3(P_all(keep_mask,1), P_all(keep_mask,2), P_all(keep_mask,3), ...
%     20, cos_vals(keep_mask), ...
%     'filled', ...
%     'MarkerEdgeColor', 'none');
% 
% colormap(cmap);
% colorbar;
% title('Voxel-colored by directional coherence');
% axis equal;
% xlabel('X'); ylabel('Y'); zlabel('Z');
% legend({'Excluded voxels', 'Kept voxels'}, 'Location', 'best');
% filename = [opt.subj_id, '_global_directional_coherence_to_only_keep_voxels_belonging_lead_', opt.traj_id];
...
% --- choose tube radius that best explains the blob
